...
from com_in_collect_trainingdata.get_faces_from_camera import TrainingDataCollector
from com_in_face_embedding.faces_embedding import GenerateFaceEmbedding
from com_in_predictor.facePredictor import FacePredictor
from com_in_training.train_softmax import TrainFaceRecogModel
...
class RegistrationModule:
    def __init__(self, logFileName):
        ...
        clientID = tk.Label(self.window, text="Client ID", width=10,
                            height=2, fg="white", bg="#363e75", font=('times', 15))
        ...
        empID = tk.Label(self.window, text="EmpID", width=10,
                         fg="white", bg="#363e75", height=2, font=('times', 15))
        empID.place(x=450, y=80)

        self.empIDTxt = tk.Entry(
            self.window, width=20, bg="white", fg="black", font=('times', 15, ' bold '))
        self.empIDTxt.place(x=575, y=80)

        empName = tk.Label(self.window, text="Emp Name", width=10,
                           fg="white", bg="#363e75", height=2, font=('times', 15))
        empName.place(x=80, y=140)

        self.empNameTxt = tk.Entry(
            self.window, width=20, bg="white", fg="black", font=('times', 15, ' bold '))
        self.empNameTxt.place(x=205, y=140)

        emailId = tk.Label(self.window, text="Email ID :", width=10,
                           fg="white", bg="#363e75", height=2, font=('times', 15))
        emailId.place(x=450, y=140)

        self.emailIDTxt = tk.Entry(
            self.window, width=20, bg="white", fg="black", font=('times', 15, ' bold '))
    ...
    def collectUserImageForRegistration(self):
        ...
        notifctn = "We have collected " + \
            str(args["faces"]) + " images for training."
    ...
    def getFaceEmbedding(self):
        ...
        ap.add_argument(
            "--embeddings", default="faceEmbeddingModels/embeddings.pickle")
        # Argument of insightface
        ap.add_argument('--image-size', default='112,112', help='')
        ap.add_argument(
            '--model', default='../insightface/models/model-y1-test2/model,0', help='path to load model.')
        ap.add_argument('--ga-model', default='', help='path to load model.')
        ap.add_argument('--gpu', default=0, type=int, help='gpu id')
        ap.add_argument('--det', default=0, type=int,
                        help='mtcnn option, 1 means using R+O, 0 means detect from begining')
        ap.add_argument('--flip', default=0, type=int,
                        help='whether do lr flip aug')
        ap.add_argument('--threshold', default=1.24,
